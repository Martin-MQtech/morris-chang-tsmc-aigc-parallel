# -*- coding: utf-8 -*-
"""
通过 createAudio 页面新建合集并全量录入 19 期
"""
import os, sys, time
...
def create_and_bind():
    with sync_playwright() as p:
        ...
        # 1. 打开 createAudio 页面创建合集
        print("📦 [1/3] 打开 createAudio 页面创建新合集...")
        ...
        new_col_btn = page.locator(".weui-desktop-dropdown__list-item:has-text('新建合集'), li:has-text('新建合集'), button:has-text('新建合集'), :text('+ 新建合集')").first
        new_col_btn.click()
        time.sleep(2.5)
        ...
        # 2. 进入合集详情页全量录入 19 期音频
        print("\n📂 [2/3] 进入合集详情并添加全量 19 期定版音频...")
        ...
        shot = os.path.join(COOKIE_PROFILE_DIR, "final_verified_19_order.png")
