# -*- coding: utf-8 -*-
"""
彻底翻页清理历史冗余单集，锁定唯一 19/19 完美正序合集
"""
import os, sys, time
...
def purge_and_lock():
    with sync_playwright() as p:
        ...
        for iteration in range(20):
            ...
            for p_idx in range(6):
                ...
                if next_pg.is_visible() and not next_pg.get_attribute("class").find("disabled") != -1:
                    next_pg.click(); time.sleep(2)
                else:
                    break

            if not deleted_in_loop:
                print("   ✅ 历史旧版单集已全部清理完毕，无任何残留！")
                break

        # 2. 重置合集内容
        print("\n📦 [2/3] 重置并重建《TSMC & Morris Chang: Parallel World》合集...")
        page.goto("https://channels.weixin.qq.com/platform/post/audioManager", wait_until="domcontentloaded")
        time.sleep(3)
        ...
        shot = os.path.join(COOKIE_PROFILE_DIR, "flawless_order_00_to_18.png")
